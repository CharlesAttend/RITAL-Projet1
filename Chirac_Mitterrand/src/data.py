...
def load_data(path="./data/raw/corpus.tache1.learn.utf8"):
    alltxts = []
    alllabs = []
...
class Custom_analyzer():
    def __init__(self, config_name) -> None:
        ...
        """
        self.config = DotDict(importlib.import_module(
            f'configs.{config_name}').config)
        self.base_analyzer = CountVectorizer(
            **self.config.base_vectorizer_param).build_analyzer()
    ...
    def __call__(self, doc):
        ...
        if self.config.number != None:
            doc = re.sub(self.config.number.regex,
                         self.config.number.replacement, doc)
